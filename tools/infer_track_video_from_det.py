...
# ------------------------------------------------------------------------------
def main():
    from cvut import video as vd
    from cvut import logger as logg
    from cvut import image as imgg
    ...
    # input video
    cap, (width, height), num_frames, fps = vd.get_video(args.video)
    print(
    ...
    if args.save_video:
        out_video_file = os.path.join(args.out_dir,
                                      logg.basename(args.video, wo_fmt=True))
        out_video_file = out_video_file + '.mp4'
        out = vd.create_video(out_video_file, (width, height), fps=fps)
    ...
    # save predictions
    if args.save_pred:
        basename = logg.basename(args.video, wo_fmt=True)

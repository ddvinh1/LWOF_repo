...
def main():
    ...
    from cvut import video as vidd
    # get video_files and pkl_files
    video_files = imgg.glob_files(args.video_dir, 'mp4')
    pkl_files = [f"{args.pkl_dir}/{logg.basename(video_file, wo_fmt=True)}.pkl"
    ...
    for pkl_id, item in enumerate(zip(video_files, pkl_files)):
        ...
        # read video
        cap, (W, H), num_frames, fps = vidd.get_video(video_file)
